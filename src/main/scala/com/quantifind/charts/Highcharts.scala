...
object Highcharts extends IterablePairLowerPriorityImplicits with BinnedDataLowerPriorityImplicits with HighchartsStyles {
  ...
  def histogram_custom[A: Numeric](data: Iterable[A], numBins: Int, min: Double, max: Double) = {
    def bin(points: Seq[Double], min: Double, max: Double): Seq[(String, Int)] = {
      ...
      def labelbin(i: Integer) = (min + step * i).toString() + " - " + (min + step * (i + 1))

      (0 until numBins).map(i => (labelbin(i), bins(i)))
    }
    
    
    def numericToDouble[X](x: X)(implicit ev: Numeric[X]): Double = ev.toDouble(x)

    val b = bin(data.map(numericToDouble(_)).toSeq, 0.0, 10.0).map(p=>(p._1,p._2.toDouble))

    plot(Histogram.histogram(b.toSeq))
  }
  ...
  def help(): Unit = {
    ...
    ListMap(
      ...
      List("unstack") -> "remove stacking").foreach {
        case (methodLines, description) =>
    ...
    ListMap(
      ...
      "deleteAll" -> "wipes all plots from the page").foreach { case (method, description) => println("\t%-35s%s".format(method, description)) }
  }
}
